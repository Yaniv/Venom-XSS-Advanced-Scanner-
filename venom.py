...
"""
    ...
    parser.add_argument("url", help="Target URL to scan (e.g., http://example.com). Required unless provided via --post-file.")
    parser.add_argument("-w", "--workers", type=int, default=5, help="Number of concurrent threads for scanning (default: 5). Increase for faster scans, decrease for stealth (e.g., 2).")
    parser.add_argument("--ai-assist", action="store_true", help="Enable AI-driven payload optimization for smarter XSS detection. Requires --ai-key for full functionality.")
    parser.add_argument("--ai-key", type=str, default=None, help="API key for AI assistance (e.g., 'your-xai-key'). Needed with --ai-assist for external AI models like xai-grok.")
    parser.add_argument("--ai-model", type=str, default="xai-grok", help="AI model for payload optimization (default: 'xai-grok'). Options: 'xai-grok', 'openai-gpt3'.")
    parser.add_argument("--scan-xss", action="store_true", help="Enable XSS scanning. Required flag to initiate the scan.", required=True)
    parser.add_argument("--payloads-dir", default="./payloads/", help="Directory containing custom payload files (default: './payloads/'). Files like 'advanced_xss.txt' or 'waf_bypass.txt' are loaded.")
    parser.add_argument("--timeout", type=int, default=10, help="HTTP request timeout in seconds (default: 10). Increase for slow servers (e.g., 20).")
    parser.add_argument("--verbose", action="store_true", help="Enable detailed logging to venom.log and console for debugging (e.g., response snippets).")
    parser.add_argument("--stealth", action="store_true", help="Force stealth mode: limits workers to 2, sets delays to 5-15s for evasion. Default: False.")
    parser.add_argument("--min-delay", type=float, help="Minimum delay between tests in seconds (e.g., 1.0). Default: 0.1 (normal), 5 (stealth).")
    parser.add_argument("--max-delay", type=float, help="Maximum delay between tests in seconds (e.g., 2.0). Default: 0.5 (normal), 15 (stealth).")
    parser.add_argument("--full-report", action="store_true", help="Show all vulnerabilities in the report, not just the first 10. Useful for large scans.")
    parser.add_argument("-H", action='append', help="Custom HTTP headers (e.g., 'Cookie: session=abc123', 'Authorization: Bearer token'). Can be specified multiple times.")
    parser.add_argument("--method", choices=['get', 'post', 'both'], default='both', help="HTTP method to scan: 'get' (query strings), 'post' (forms), 'both' (default).")
    parser.add_argument("--data", type=str, default=None, help="POST request data (e.g., 'key1=value1&key2=value2'). Overrides --post-file data if both are set.")
    parser.add_argument("--post-file", type=str, default=None, help="Path to TXT file with POST request (e.g., 'post.txt'). Format: 'POST /path\nHeader: Value\n\nkey=value'.")
    parser.add_argument("--payload-field", type=str, default=None, help="Specific field to inject payloads into (e.g., 'search'). Overrides default injection points.")
    parser.add_argument("--login-url", type=str, default=None, help="URL for login to establish a session (e.g., 'http://example.com/login'). Requires --login-data.")
    parser.add_argument("--login-data", type=str, default=None, help="Login credentials for POST (e.g., 'username=admin&password=pass123'). Used with --login-url.")
    parser.add_argument("--auto-login", action="store_true", help="Automatically detect login pages and attempt login with default credentials (e.g., admin/admin). Default: False.")
